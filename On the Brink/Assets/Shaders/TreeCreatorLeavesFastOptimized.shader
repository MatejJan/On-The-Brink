...
Properties {
    ...
    _Threshold3("Threshold 3", Range(0, 1)) = 0.4

    _Color4("Color 4", Color) = (1, 1, 1, 1)
    _Threshold4("Threshold 4", Range(0, 1)) = 0.2
...
SubShader {
    ...
    CGPROGRAM
        ...
        fixed4 FragmentLeaf (v2f_leaf IN) : SV_Target
        {
            ...
            float lightIntensity = light.r * albedo.r;

            if (lightIntensity > _Threshold1) {
                return fixed4(_Color1.rgb, 0.0);
                }
            else if (lightIntensity > _Threshold2) {
                return fixed4(_Color2.rgb, 0.0);
                }
            else if (lightIntensity > _Threshold3) {
                return fixed4(_Color3.rgb, 0.0);
                }
            else if (lightIntensity > _Threshold4) {
                return fixed4(_Color4.rgb, 0.0);

            } else {
                return fixed4(_Color5.rgb, 0.0);
            }
        }
