Shader "FlexibleCelShader/Cel Water"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert(appdata_tan v)
			{
				...
				// Position data
				float waveOffsetZ = _Time.g * 0.2 + v.vertex.z * 2;
				float waveOffsetX = _Time.g * 0.1 + (v.vertex.x * 0.1 + v.vertex.z * 0.5);
				float waveFactor = 0.4;
				v.vertex.y = cos(waveOffsetZ) * waveFactor;

				o.worldPos = mul(unity_ObjectToWorld, v.vertex);
				o.pos = mul(UNITY_MATRIX_VP, o.worldPos);

				// Normal data
				v.normal.z = -sin(waveOffsetZ) - v.vertex.z * 0.03;
				v.normal.x = sin(waveOffsetX);
				v.normal.y = 1;
